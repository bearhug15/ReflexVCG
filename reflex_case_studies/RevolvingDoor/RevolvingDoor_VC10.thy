...
next
  ...
  proof (simp only:extraSuspendedOut_def;rule allI; rule impI)
    ...
    proof (cases)
      ...
      have buff:"getPstate st0 ''Controller'' = ''suspended''" using assms by auto
      have "\<not> getVarBool st0 ''rotation''" using buff base_inv extraInv_def extraSuspendedOut_def substate_refl by blast
      then have 2:"\<not> getVarBool st_final ''rotation''" using assms by auto
      have "getVarBool st0 ''brake''" using buff base_inv extraInv_def extraSuspendedOut_def substate_refl by blast
      then have 3:"getVarBool st_final ''brake''" using assms by auto
...
next
  ...
  proof(simp only: extra4_def;rule allI;rule impI)
    ...
    proof (cases)
      assume 1:"s1=st_final"
      have 2:"\<not>getVarBool st_final ''pressure''" using assms by auto
      have 3:"getPstate st0 ''Controller'' = ''suspended''" using assms by auto
      then have 4:"(\<exists> s2 s3. 
      toEnvP s2 \<and> 
      toEnvP s3 \<and> 
      substate s2 s3 \<and> 
      substate s3 st0 \<and> 
      ...
      (getPstate s2 ''Controller'' \<noteq> ''motionless'' \<or> 
        getVarBool s3 ''user'') \<and> 
      getVarBool s3 ''pressure'')" using assms extraInv_def extra4_def substate_refl by auto
      have 5:"substate st0 st_final" using assms substate_refl by auto
      have 6:"extra4 st0" using assms extraInv_def by auto
      show ?thesis using 1 2 3 4 5 6 prems substate_refl extra4_def assms extraInv_def by sledgehammer
    next
      assume 1:"s1\<noteq>st_final"
      have "predEnv st_final = st0" using assms extraInv_def by auto
      then have 2:"substate s1 st0" using 1 assms prems substate_eq_or_predEnv by blast
