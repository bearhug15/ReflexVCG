  ...
  proof -
    ...
    proof(simp only: R1_A_def R1_B_def R1_C_def SMT.verit_bool_simplify(4); rule allI; rule impI)
      ...
      proof -
        ...
        have 4:"getPstate st0 ''Controller'' = ''redAfterMinimalRed''" using assms by auto
        consider (bp)"(getVarBool st0 ''_bpressed'')"
          | (bu)"(getVarBool st_final ''button'')" using assms by auto
        thus ?thesis
        proof (cases)
          case bp
          then obtain s2 where
            o1:"toEnvP s2 \<and> substate s2 st0 \<and> 
                toEnvNum s2 st0 = 1 \<and> getPstate s2 ''Controller'' = ''minimalRed'' \<and>
                ltime s2 ''Controller'' = 10000 \<and>
                getVarBool s2 ''_bpressed''" 
            using extra1 extraInv_def extra17_def substate_refl 0 4 by blast
          have "100\<le>ltime st0 ''Controller'' " using 4 extra1 extraInv_def extra2_2_def 0 substate_refl by blast
          then consider (l1)"ltime st0 ''Controller'' div 100 =1"
            | (l2) "ltime st0 ''Controller'' div 100 >1" by force 
          then show ?thesis 
          proof (cases)
            case l1
