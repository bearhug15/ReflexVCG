...
next
  ...
  next
    ...
    proof (simp add:R1_sub1_def;rule allI;rule allI; rule impI)
      fix s1 s2
      assume prems: 
        "R1_sub1_prems st4 s1 s2"
      thus "R1_sub2 st4 s2"
      proof (simp only:R1_sub1_prems_def; cases)
        assume 1:"s2=st4"
        have "R1_sub2_prems st4 s2 s2 \<and> R1_sub3 s2 s2" 
        proof
          from 1 show "R1_sub2_prems st4 s2 s2" using R1_sub2_prems_def assms by auto
...
next
  ...
  next
    ...
    proof (simp only:R2_sub1_def; rule allI;rule allI;rule impI)
      fix s1 s2
      assume prems:"R2_sub1_prems st4 s1 s2"
      thus "getVarBool s2 ''out_1'' = False"
      proof (simp only: R2_sub1_prems_def; cases)
        assume 1: "s2=st4"
        thus ?thesis using assms prems R2_sub1_prems_def by auto
      next
...
next
  ...
  next
    ...
    proof(simp only: R3_sub1_def; intro allI; rule impI)
      fix s1 s2
      assume prems:"R3_sub1_prems st4 s1 s2" 
      then obtain "20 \<le> toEnvNum s2 st4" using R3_sub1_prems_def by auto
      ...
      proof -
        ...
        then have "R3_sub2_prems st4 s2 s4 \<and> R3_sub3 s2 s4" using assms R3_sub2_prems_def by auto
